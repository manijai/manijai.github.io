...
////////////////////////////////////////////////////////////////////////
var packagesUtil = new PackagesUtil(["animations","cloud","components","componentsPreviewLayer","core","documentServices","editingRendererPlugins","fonts","layout","previewExtensionsCore","qaAutomation","server","skins","testUtils","tpa","tpaIntegration","tweenEngine","utils","wixCode","wixCodeIntegration","wixSites","wixappsBuilder","wixappsClassics","wixappsCore"], window.location.search);
...
function getViewerRjsConfig (serviceTopology) { //eslint-disable-line no-unused-vars
    ...
    return {
        ...
        paths: {
            ...
            react: {min: serviceURL('react/0.14.1/react-with-addons.min'), source: '//cdnjs.cloudflare.com/ajax/libs/react/0.14.1/react-with-addons'},
            reactDOM: {min: serviceURL('react/0.14.1/react-dom.min'), source: '//cdnjs.cloudflare.com/ajax/libs/react/0.14.1/react-dom'},
...
function getFullRjsConfig(rjsConfigFunc, packagesUtil, reactVersions, artifactData, serviceTopology) {
    ...
    if (reactVersions) {
        ...
        config.baseUrl = artifactPath;
    } else if (isAddress(artifactData.versionsParam)) {
        config.baseUrl = artifactData.versionsParam.replace('/target', '');
    } else if (artifactData.baseVersionOverride) {
...
function render(isServerSide, isPreview, contentCache, queryUtil, siteModel, wixBiSession) {
    ...
    function fixPages(p, pagesData) {
        ...
        return _.mapValues(pagesData, function (data, pageId) {
            ...
            return p.utils.dataFixer.fix(data, pageIds.slice(), siteModel.requestModel, siteModel.currentUrl);
        });
    }
    ...
    function renderClientSide() {
        var clientSidePackages = ['skins', 'components', 'core', 'react', 'reactDOM', 'utils', 'lodash', 'TweenMax',
            'wixappsCore', 'wixappsClassics', 'layout', 'tpa', 'zepto', 'TimelineMax', 'ScrollToPlugin', 'wixappsBuilder',
            'fonts', 'animations', 'color', 'swfobject', 'mousetrap', 'wixCode'];
...
/*global packagesUtil, instrument, queryUtil, overrideScriptsLocationMapFromQuery, prefetchPages, convertSiteModel, fixViewport, getFullRjsConfig, getViewerRjsConfig, getSubdomain, render, convertRendererModel, joinURL, addExperimentsFromQuery*/
    ...
    siteModel.santaBase = reactVersions ? joinURL(config.baseUrl, reactVersions.base) : config.baseUrl;
    serviceTopology.staticServerFallbackUrl = serviceTopology.staticServerFallbackUrl || 'https://fallback.wix.com/';
    if (reactVersions) {
        siteModel.baseVersion = reactVersions.base;
        siteModel.santaBaseFallbackUrl = joinURL(serviceTopology.staticServerFallbackUrl, reactVersions.base);
    } else {
